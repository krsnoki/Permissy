export const directorOfcOptions = [
    {to: "/", label: "Accounts"},
    {label: "Events", children: [
        {to: "/", label: "College Events"},
        {to: "/", label: "External Events"},]},
    {to: "/", label:"SIC"},
]

export const siteSectionOptions = [
    {to: "/", label: "Event Site Access"},
    {to:"/", label: "Events Props"},
]

export const deanOfcOptions = [
        {to: "/", label: "Student Activities"},
        {to:"/", label: "Finance"},
        {to:"/", label: "Academics"},
        {to:"/", label: "R & D applications"},
        {to:"/", label: "Incubation"},
        {to:"/", label: "IT services"},
        {to:"/", label: "Planning"},
]

export const deptOptions = [
    {to:"/", label: "Civil department"},
    {to:"/", label: "Mechnical department"},
    {to:"/", label: "ENTC department"},
    {to:"/", label: "Instrumentation department"},
    {to:"/", label: "Electrical department"},
    {to:"/", label: "Computer department"},
    {to:"/", label: "IT Department"},
    {to:"/", label: "Chemical department"},
    {to:"/", label: "Production department"},
    {to:"/", label: "Textile department"},
]